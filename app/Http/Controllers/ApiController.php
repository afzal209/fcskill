...
class ApiController extends Controller
{
    ...
    public function signal_check($type){
       // if($type == 0){
        ...
        // }
        if($type == 0){
            $signal_check = Signal::where('signal_type',0)->orderBy('created_at','desc')->get();
        }
        else if($type == 1){
            $signal_check = Signal::where('signal_type',1)->orderBy('created_at','desc')->get();
        }
        else{
            $signal_check = Signal::orderBy('created_at','desc')->get();
        }
    ...
    public function gain_profits_add(Request $request){
        ...
        if($request->hasFile('image')) {
            ...
            if($extension != "jpg" && $extension != "png" && $extension != "jpeg" && $extension != "jpeg" && $extension != "heic"){
                ...
                $gain_profits->save();
                return response()->json(['success' => 'Thanks for your post! <br/>it will publish soon as possible!']);
    ...
    public function gain_profits_show(Request $request){
        // // if($request->status == 2){
        //     $gain_profits = GainProfit::where('user_choice',$request->user_choice)->where('status',2)->get();
        // // }
        // return response()->json(['data' => $gain_profits]);
        if($request->status == 2){
            $gain_profits = GainProfit::where('user_choice',$request->user_choice)->where('status',$request->status)->orderBy('id','desc')->get();
            return response()->json(['data' => $gain_profits]);
        }
        else{
            return response()->json(['data' => 'Not Approve']);
        }
    }

    public function gain_profit_latest(Request $request){
        if($request->user_choice == 0){
            $gain_profits = GainProfit::where('id' , '>', $request->id)->where('user_choice','=', 0)->where('status',2)->orderBy('id','desc')->get();
            return response()->json(['data' => $gain_profits], 200);
        }
        else if($request->user_choice == 1){
            $gain_profits = GainProfit::where('id' , '>', $request->id)->where('user_choice','=', 1)->where('status',2)->orderBy('id','desc')->get();
            return response()->json(['data' => $gain_profits], 200);
        }
        else{
            $gain_profits = GainProfit::where('id' , '>', $request->id)->where('user_choice','=', 2)->where('status',2)->orderBy('id','desc')->get();
            return response()->json(['data' => $gain_profits], 200);
        }
    }
